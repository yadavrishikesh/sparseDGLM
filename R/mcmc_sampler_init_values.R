#' MCMC Sampler to run a Bayesian Regression Model With Only Fixed Effects Thar are used then to set initial values
#'
#' This function performs MCMC sampling for a Bayesian regression model with only fixed effects covariates.
#'
#' @param data_lik Character, specifying the data likelihood, e.g., `"Poisson"`, `"NegB"`, etc.
#' @param nt Integer, the number of time points in the data set.
                                 ...
                                 init.seed)
{
    ...
    if((i%%thin)==0 | i==1){  ## this is the thinning steps and adapt is the number of iterations after which i update the variance of MALA and random walk algorithms
      ...
      if(print.result==TRUE){
        if(i< (burn_in1+burn_in2+2)){
          cat(paste0("Bayesian GLM to Set Good Initial Values", "\n",
                     " Iteration: ",i, "\n",
  ...
  beta.samples<- if(data_lik=="NegB" | data_lik=="lNormal"){samples[-(1:(burn_in1/thin + burn_in2/thin)), 4:(3+q)]} else{samples[-(1:(burn_in1/thin + burn_in2/thin)), 3:(2+q)]}
  sumry.beta<- apply(beta.samples, MARGIN = 2, FUN = mean)

  return(
    list("sumry.beta" = sumry.beta,
